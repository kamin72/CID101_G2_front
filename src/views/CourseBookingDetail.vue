<template>
  ...
  <section class="course-booking-section" v-if="course">
    <div class="course-booking-container">
      ...
      <RouterLink
        :to="'/courseBookingDetail_confirm/' + course.course_id"
        style="text-decoration: none"
      >
        <button
          :disabled="disabledButton"
          :class="{
            'big-btn-invalid': disabledButton,
            'big-btn-primary': false,
            reserveCourse: true
          }"
    ...
    async fetchDiscounts() {
      try {
        ...
        const response = await fetch(
          `${import.meta.env.VITE_API_URL}/front/getMemberCoupon.php?memberId=${memberId}`
        )
