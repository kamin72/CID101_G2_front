  ...
  <section class="section-recom-course">
    <div class="course-page-container">
      <div class="row">
        ...
        <!-- 推薦卡片清單 -->
        <div v-for="(course, index) in recommendedCourses" :key="index" class="col-3 col-md-4 col-sm-6 event-card1">
          ...
          <div class="col-11 search-course-list-wrap">
            <div class="search-course-tabs-panel">
              <div class="search-course-calen-tab">
                <div @click="showCalen = true; showList = false; toggleButton('calen')"
                  :class="{ 'big-btn-primary': activeButton === 'calen', 'big-btn-secondary': activeButton !== 'calen' }">
                  從日曆中選擇
                  <div class="triangle"></div>
                </div>
              </div>
              <div class="search-course-list-tab">
                <div @click="showCalen = false; showList = true; toggleButton('list')"
                  :class="{ 'big-btn-primary': activeButton === 'list', 'big-btn-secondary': activeButton !== 'list' }">
                  從清單中選擇
                  <div class="triangle"></div>
                </div>
              </div>
              ...
              <div class="search-course-calendar-body">
                ...
                <div class="days">
                  ...
                  <div v-for="day in days" :key="day.date" class="day"
                    :class="{ today: day.isToday, 'has-course': hasCourse(day.date) }">
                    ...
                    <div v-if="hasCourse(day.date)" class="course-name">
                      ...
                      <p :style="{ paddingTop: getCourseTagByDate(day.date) ? '' : 'unset' }">
            ...
            <div v-show="showList" class="col-7 col-md-7 col-sm-12 event-card-wrap">
              <div class="search-course-calendar-header">
                <span class="prev material-symbols-outlined" @click="prevListMonth">arrow_back_ios</span>
                <h4 class="month-year">{{ currentListDate.getFullYear() }}.{{ currentListDate.getMonth() + 1 }}</h4>
                <span class="next material-symbols-outlined" @click="nextListMonth">arrow_forward_ios</span>
              </div>
              <div v-for="([date, course], index) in getMonthCourses(currentListDate)" :key="index" class="event-card2">
...
export default {
  ...
  methods: {
    ...
    prevListMonth() {
      this.currentListDate = new Date(this.currentListDate.getFullYear(), this.currentListDate.getMonth() - 1);
    },
    nextListMonth() {
      this.currentListDate = new Date(this.currentListDate.getFullYear(), this.currentListDate.getMonth() + 1);
    },
    ...
    getRecommendedCourses() {
      ...
      const futureCoursesArray = Array.from(this.courses).filter(([date]) => new Date(date) >= currentDate);

      if (futureCoursesArray.length <= 4) {
        this.recommendedCourses = futureCoursesArray.map(([date, course]) => ({ ...course, date }));
      } else {
        const shuffledCourses = futureCoursesArray.sort(() => 0.5 - Math.random());
        this.recommendedCourses = shuffledCourses.slice(0, 4).map(([date, course]) => ({ ...course, date }));
      }
    },
  },

  created() {
    this.courses.set('2024-06-13', { img: '1.jpg', tag: null, name: '品酒初級課程1', price: 3200, desc: '課程內容包括葡萄酒的基本知識、品酒技巧、如何選酒等。透過理論與實作快速掌握品酒的要領，開啟探索葡萄酒世界的大門。' })
    this.courses.set('2024-06-15', { img: '2.jpg', tag: null, name: '品酒中級課程1', price: 3600, desc: '深入探討葡萄酒風土、品種、釀造工藝及熟成，並透過系統化品評訓練，提升品酒技巧。' })
    this.courses.set('2024-07-05', { img: '3.jpg', tag: null, name: '品酒進階課程1', price: 4200, desc: '深入剖析頂級葡萄酒的特色，探討稀有品種與產區，透過大師級講師的指導，磨練品酒技藝成為真正的品酒專家。' })
    this.courses.set('2024-07-30', { img: '4.jpg', tag: '早鳥優惠', name: '品酒初級課程2', price: 3800, desc: '進一步探索葡萄酒的風味特性。透過品嚐不同產區與風格的葡萄酒，訓練您的味蕾，學習如何描述與分享品酒心得。' })
