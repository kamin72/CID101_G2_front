  ...
  async mounted() {
    try {
      const [courseResponse, newsResponse] = await Promise.all([
        fetch(`${import.meta.env.VITE_API_URL}/front/home/courseSwiper.php`).then((response) =>
          response.json()
        ),
        fetch(`${import.meta.env.VITE_API_URL}/front/news.php`).then((response) => response.json())
      ])
